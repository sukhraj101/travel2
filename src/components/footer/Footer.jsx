// Footer.js
import React from 'react';
import logofivvia from "./../../../public/assets/img/logo/fivvia.png"

const Footer = () => {
  const socialMediaLinks = [
    {
      icon: 'facebook-f',
      link: '#',
    },
    {
      icon: 'twitter',
      link: '#',
    },
    {
      icon: 'instagram',
      link: '#',
    },
    {
      icon: 'linkedin-in',
      link: '#',
    },
  ];

  const footerLinks = [
    {
      title: 'Travila',
      links: [
        {
          text: 'About us',
          link: '/about-us',
        },
        {
          text: 'Contact Us',
          link: '/contact-us',
        },
        {
          text: 'Terms of Use',
          link: '/terms',
        },
        {
          text: 'Privacy Statement',
          link: '/privacy',
        },
        {
          text: 'Give Us Feedbacks',
          link: '/feedback',
        },
        {
          text: 'FAQs',
          link: '/faq',
        },
      ],
    },
    {
      title: 'Featured Tours',
      links: [
        {
          text: 'Honeymoon Tours',
          link: '/tour-list',
        },
        {
          text: 'Adventure Tours',
          link: '/tour-list',
        },
        {
          text: 'Pilgrimage Tours',
          link: '/tour-list',
        },
        {
          text: 'Cultural Tours',
          link: '/tour-list',
        },
        {
          text: 'Luxury Tours',
          link: '/tour-list',
        },
        {
          text: 'Wildlife Tours',
          link: '/tour-list',
        },
      ],
    },
    {
      title: 'Activities',
      links: [
        {
          text: 'Hiking',
          link: '/tour-list',
        },
        {
          text: 'Cycling',
          link: '/tour-list',
        },
        {
          text: 'Snorkeling',
          link: '/tour-list',
        },
        {
          text: 'Skydiving',
          link: '/tour-list',
        },
        {
          text: 'Surfing',
          link: '/tour-list',
        },
        {
          text: 'Zip-lining',
          link: '/tour-list',
        },
      ],
    },
  ];

  const paymentIcons = [
    {
      src: '../../assets/img/icons/img3.png',
      alt: 'Payment Icons',
    },
    {
      src: '../../assets/img/icons/img4.png',
      alt: 'Payment Icons',
    },
    {
      src: '../../assets/img/icons/img5.png',
      alt: 'Payment Icons',
    },
    {
      src: '../../assets/img/icons/img6.png',
      alt: 'Payment Icons',
    },
    {
      src: '../../assets/img/icons/img7.png',
      alt: 'Payment Icons',
    },
  ];

  const languageOptions = [
    {
      value: 'one',
      text: 'English(United States)',
    },
    {
      value: 'one',
      text: 'English(United Kingdom)',
    },
    {
      value: 'one',
      text: 'English',
    },
    {
      value: 'two',
      text: 'Deutsch',
    },
    {
      value: 'two',
      text: 'Francais',
    },
    {
      value: 'four',
      text: 'Espanol',
    },
  ];

  const currencyOptions = [
    {
      value: 'one',
      text: '$ USD',
    },
    {
      value: 'two',
      text: '€ EUR',
    },
    {
      value: 'three',
      text: '₺ TL',
    },
    {
      value: 'four',
      text: '₽ RUB',
    },
  ];

  const bottomLinks = [
    {
      text: 'Tours',
      link: '/tour-list',
    },
    {
      text: 'Destinations',
      link: '/tour-list',
    },
    {
      text: 'Activities',
      link: '/tour-list',
    },
    {
      text: 'Yachts',
      link: '/tour-list',
    },
  ];

  return (
    <footer className="footer mt-4">
      <div className="space-bottom-2 space-top-1">
        <div className="container">
          <div className="row justify-content -xl-between">
            <div className="col-12 col-lg-4 col-xl-3dot1 mb-6 mb-md-10 mb-xl-0">
              <div className="d-md-flex d-lg-block">
                <div className="mb-5 mr-md-7 mr-lg-0">
                  <h4 className="h6 mb-4 font-weight-bold">Need My Travel Help?</h4>
                  <a href="tel:(888)-1234-56789">
                    <div className="d-flex align-items-center">
                      <div className="mr-4">
                        <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="40px" height="40px">
                          <path fill-rule="evenodd" className="fill-primary" d="M36.093,16.717 L33.133,16.717 L30.864,19.693 C30.717,19.886 30.487,20.000 30.244,20.000 C30.244,20.000 30.243,20.000 30.243,20.000 C30.000,20.000 29.771,19.888 29.623,19.695 L27.335,16.717 L24.372,16.717 C22.218,16.717 20.465,14.965 20.465,12.811 L20.465,3.907 C20.465,1.753 22.218,0.001 24.372,0.001 L36.093,0.001 C38.247,0.001 40.000,1.753 40.000,3.907 L40.000,12.811 C40.000,14.965 38.247,16.717 36.093,16.717 ZM38.437,3.907 C38.437,2.615 37.386,1.563 36.093,1.563 L24.372,1.563 C23.080,1.563 22.028,2.615 22.028,3.907 L22.028,12.811 C22.028,14.103 23.080,15.155 24.372,15.155 L27.721,15.155 C27.964,15.155 28.193,15.268 28.340,15.460 L30.240,17.934 L32.124,15.462 C32.272,15.269 32.502,15.155 32.746,15.155 L36.093,15.155 C37.386,15.155 38.437,14.103 38.437,12.811 L38.437,12.811 L38.437,3.907 ZM31.014,8.429 L31.014,9.647 C31.014,10.078 30.664,10.428 30.232,10.428 C29.801,10.428 29.451,10.078 29.451,9.647 L29.451,8.239 C29.451,7.618 29.876,7.089 30.485,6.953 C31.041,6.829 31.416,6.323 31.376,5.752 C31.337,5.186 30.881,4.730 30.316,4.691 C29.992,4.669 29.685,4.777 29.451,4.996 C29.216,5.216 29.086,5.514 29.086,5.834 C29.086,6.266 28.736,6.616 28.305,6.616 C27.873,6.616 27.523,6.266 27.523,5.834 C27.523,5.087 27.837,4.365 28.384,3.854 C28.939,3.336 29.663,3.081 30.423,3.132 C31.763,3.225 32.843,4.304 32.935,5.644 C33.024,6.927 32.225,8.068 31.014,8.429 ZM30.233,11.646 C30.438,11.646 30.640,11.729 30.785,11.874 C30.930,12.020 31.014,12.221 31.014,12.427 C31.014,12.633 30.930,12.834 30.785,12.980 C30.640,13.125 30.438,13.209 30.233,13.209 C30.027,13.209 29.825,13.125 29.680,12.980 C29.535,12.834 29.451,12.633 29.451,12.427 C29.451,12.221 29.535,12.020 29.680,11.874 C29.825,11.729 30.027,11.646 30.233,11.646 ZM14.440,16.019 L23.973,25.550 L24.760,24.764 C24.760,24.764 24.760,24.764 24.760,24.764 C24.760,24.764 24.761,24.763 24.761,24.763 L26.731,22.794 C27.574,21.951 28.695,21.487 29.887,21.487 C31.079,21.487 32.200,21.951 33.043,22.794 L38.693,28.442 C39.536,29.285 40.000,30.406 40.000,31.598 C40.000,32.790 39.536,33.910 38.693,34.753 L37.204,36.242 C34.780,38.665 31.557,40.000 28.129,40.000 C24.701,40.000 21.478,38.665 19.054,36.242 L13.486,30.676 C13.181,30.370 13.181,29.875 13.486,29.570 C13.792,29.265 14.286,29.265 14.592,29.570 L20.159,35.137 C22.288,37.265 25.118,38.438 28.129,38.438 C30.646,38.438 33.038,37.617 34.998,36.104 L25.313,26.421 L24.526,27.208 C24.221,27.513 23.726,27.513 23.421,27.208 L12.782,16.572 C12.636,16.425 12.553,16.227 12.553,16.019 C12.553,15.812 12.636,15.614 12.782,15.467 L16.092,12.158 C17.223,11.027 17.223,9.187 16.092,8.056 L10.448,2.413 C9.317,1.282 7.476,1.282 6.345,2.413 L4.928,3.830 L10.929,9.830 C11.234,10.135 11.234,10.630 10.929,10.935 C10.776,11.087 10.576,11.163 10.376,11.163 C10.176,11.163 9.976,11.087 9.824,10.935 L3.890,5.002 C0.490,9.418 0.812,15.794 4.856,19.837 L10.346,25.326 C10.651,25.631 10.651,26.126 10.346,26.431 C10.041,26.736 9.546,26.736 9.241,26.431 L3.751,20.943 C-1.253,15.940 -1.253,7.800 3.751,2.797 L5.240,1.308 C6.981,-0.432 9.813,-0.432 11.553,1.308 L17.197,6.951 C18.938,8.691 18.938,11.522 17.197,13.263 L14.440,16.019 ZM36.170,35.066 L37.588,33.648 C38.135,33.101 38.437,32.372 38.437,31.598 C38.437,30.823 38.135,30.094 37.587,29.547 L31.938,23.899 C31.390,23.351 30.662,23.049 29.887,23.049 C29.112,23.049 28.384,23.351 27.836,23.899 L26.418,25.316 L36.170,35.066 ZM11.492,12.280 C11.492,12.074 11.576,11.873 11.721,11.727 C11.866,11.581 12.068,11.498 12.274,11.498 C12.479,11.498 12.681,11.581 12.826,11.727 C12.971,11.873 13.055,12.074 13.055,12.280 C13.055,12.485 12.971,12.687 12.826,12.832 C12.681,12.977 12.479,13.061 12.274,13.061 C12.068,13.061 11.866,12.977 11.721,12.832 C11.576,12.687 11.492,12.485 11.492,12.280 ZM12.046,27.349 C12.252,27.349 12.453,27.433 12.599,27.578 C12.744,27.724 12.828,27.925 12.828,28.130 C12.828,28.336 12.744,28.538 12.599,28.683 C12.453,28.828 12.252,28.912 12.046,28.912 C11.841,28.912 11.639,28.828 11.494,28.683 C11.348,28.538 11.265,28.336 11.265,28.130 C11.265,27.925 11.348,27.724 11.494,27.578 C11.639,27.433 11.841,27.349 12.046,27.349 Z"></path>
                        </svg>
                      </div>
                    <div>
                      <div className="mb-2 h6 font-weight-normal text-gray-1">
                        Got Questions? Call us 24/7
                      </div>
                      <small className="d-block font-size-18 font-weight-normal text-primary">
                        Call Us: <span className="text-primary font-weight-semi-bold">(888) 1234 56789</span>
                      </small>
                    </div>
                  </div>
                </a>
              </div>
              <div className="ml-md-6 ml-lg-0">
                <div className="mb-4">
                  <h4 className="h6 font-weight-bold mb-2 font-size-17">
                    Contact Info
                  </h4>
                  <address className="pr-4">
                    <span className="mb-2 h6 font-weight-normal text-gray-1">
                      PO Box CT16122 Collins Street West, Victoria 8007,
                      Australia.
                    </span>
                  </address>
                </div>
                <ul className="list-inline mb-0">
                  <li className="list-inline-item mr-2">
                    <a className="btn btn-icon btn-social btn-bg-transparent" href="https://fivvia.com/">
                      <span>
                        <img src={logofivvia} alt="fivvia logo" />
                      </span>
                    </a>
                  </li>
                  {socialMediaLinks.map((link, index) => (
                    <li key={index} className="list-inline-item mr-2">
                      <a
                        className="btn btn-icon btn-social btn-bg-transparent"
                        href={link.link}
                      >
                        <span className={`fab fa-${link.icon} btn-icon__inner`}></span>
                      </a>
                    </li>
                  ))}
                </ul>
              </div>
            </div>
          </div>
          <div className="col-12 col-md-6 col-lg-4 col-xl-1dot8 mb-6 mb-md-10 mb-xl-0">
            <h4 className="h6 font-weight-bold mb-2 mb-xl-4">Travila</h4>
            <ul className="list-group list-group-flush list-group-borderless mb-0">
              {footerLinks[0].links.map((link, index) => (
                <li key={index}>
                  <a
                    className="text-decoration-on-hover list-group-item list-group-item-action"
                    href={link.link}
                  >
                    {link.text}
                  </a>
                </li>
              ))}
            </ul>
          </div>
          <div className="col-12 col-md-6 col-lg-4 col-xl-1dot8 mb-6 mb-md-10 mb-xl-0">
            <h4 className="h6 font-weight-bold mb-2 mb-xl-4">Featured Tours</h4>
            <ul className="list-group list-group-flush list-group-borderless mb-0">
              {footerLinks[1].links.map((link, index) => (
                <li key={index}>
                  <a
                    className="list-group-item list-group-item-action text-decoration-on-hover"
                    href={link.link}
                  >
                    {link.text}
                  </a>
                </li>
              ))}
            </ul>
          </div>
          <div className="col-12 col-md-6 col-lg-4 col-xl-1dot8 mb-6 mb-md-0">
            <h4 className="h6 font-weight-bold mb-2 mb-xl-4">Activities</h4>
            <ul className="list-group list-group-flush list-group-borderless mb- 0">
              {footerLinks[2].links.map((link, index) => (
                <li key={index}>
                  <a
                    className="list-group-item list-group-item-action text-decoration-on-hover"
                    href={link.link}
                  >
                    {link.text}
                  </a>
                </li>
              ))}
            </ul>
          </div>
          <div className="col-12 col-md-6 col-lg col-xl-3dot1">
            <div className="mb-4 mb-xl-2">
              <h4 className="h6 font-weight-bold mb-2 mb-xl-4">Mailing List</h4>
              <p className="m-0 text-gray-1">
                Sign up for our mailing list to get latest updates and offers.
              </p>
            </div>
            <form
              className="js-validate js-focus-state js-form-message"
              noValidate
            >
              <label
                className="sr-only text-gray-1"
                htmlFor="subscribeSrEmailExample1"
              >
                Your Email
              </label>
              <div className="input-group">
                <input
                  type="email"
                  className="form-control height-54 font-size-14 border-radius-3 border-width-2 border-color-8"
                  name="email"
                  id="subscribeSrEmailExample1"
                  placeholder="Your Email"
                  aria-label="Your email address"
                  aria-describedby="subscribeButtonExample3"
                  required
                  data-msg="Please enter a valid email address."
                  data-error-className="u-has-error"
                  data-success-className="u-has-success"
                />
                <div className="input-group-append ml-3">
                  <button
                    className="btn btn-primary border-radius-3 height-54 min-width-112 font-size-14"
                    type="submit"
                    id="subscribeButtonExample3"
                  >
                    Subscribe
                  </button>
                </div>
              </div>
            </form>
          </div>
          </div> 
        </div>
      </div>
      <div className="border-top border-bottom border-color-8 space-1">
        <div className="container">
          <div className="row d-block d-xl-flex align-items-md-center">
            <div className="col-xl-4 mb-4 mb-xl-0">
              <a
                className="d-inline-flex align-items-center"
                href="/"
                aria-label="Travila"
              >
                <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="55px" height="53px">
                    <path fill-rule="evenodd" className="fill-primary" d="M53.175,51.207 C50.755,53.610 46.848,53.594 44.448,51.172 L40.766,47.484 C40.378,47.082 40.378,46.444 40.766,46.041 C41.164,45.628 41.821,45.617 42.233,46.016 L45.915,49.702 C47.503,51.246 50.030,51.246 51.619,49.703 C53.243,48.125 53.283,45.528 51.708,43.902 L50.100,42.291 C49.712,41.889 49.712,41.251 50.100,40.849 C50.498,40.436 51.155,40.425 51.567,40.823 L53.174,42.433 C53.186,42.445 53.198,42.457 53.210,42.469 C55.610,44.892 55.594,48.804 53.175,51.207 ZM47.857,37.404 C47.757,37.404 47.657,37.389 47.561,37.360 C47.561,37.360 47.561,37.360 47.561,37.360 C47.012,37.196 46.700,36.618 46.864,36.068 C48.542,30.413 47.740,24.309 44.659,19.281 C38.665,9.497 25.886,6.432 16.116,12.435 C16.085,12.456 16.054,12.476 16.021,12.493 C15.518,12.767 14.888,12.581 14.614,12.077 C14.340,11.574 14.526,10.943 15.029,10.669 C18.623,8.455 22.761,7.284 26.981,7.288 C29.178,7.289 31.363,7.608 33.469,8.235 C45.556,11.831 52.442,24.559 48.851,36.662 C48.719,37.102 48.315,37.404 47.857,37.404 ZM13.802,8.022 L12.765,6.984 C12.377,6.581 12.377,5.943 12.765,5.540 C13.163,5.128 13.820,5.116 14.232,5.515 L15.269,6.554 C15.657,6.956 15.657,7.594 15.269,7.996 C14.871,8.409 14.214,8.420 13.802,8.022 ZM9.654,3.868 L9.084,3.297 C7.495,1.753 4.968,1.753 3.379,3.296 C1.755,4.874 1.715,7.470 3.291,9.097 L10.083,15.900 C10.278,16.095 10.387,16.358 10.387,16.634 C10.387,17.207 9.923,17.672 9.350,17.672 C9.075,17.672 8.812,17.563 8.617,17.368 L1.824,10.566 C1.812,10.554 1.800,10.542 1.788,10.530 C-0.611,8.108 -0.596,4.195 1.824,1.792 C4.243,-0.611 8.150,-0.595 10.550,1.827 L11.121,2.399 C11.129,2.408 11.138,2.417 11.146,2.425 C11.544,2.837 11.533,3.495 11.121,3.893 C10.709,4.292 10.052,4.281 9.654,3.868 ZM7.742,19.850 C8.260,20.096 8.480,20.715 8.234,21.233 C5.232,27.580 5.635,35.016 9.305,41.001 C15.302,50.780 28.080,53.839 37.845,47.834 C37.876,47.813 37.908,47.793 37.940,47.775 C38.444,47.501 39.073,47.687 39.347,48.191 C39.621,48.695 39.435,49.325 38.932,49.600 C35.338,51.814 31.200,52.984 26.981,52.981 C23.606,52.979 20.273,52.228 17.223,50.782 C5.829,45.380 0.966,31.752 6.360,20.342 C6.606,19.824 7.225,19.604 7.742,19.850 ZM40.262,35.347 C40.601,35.281 40.951,35.387 41.196,35.631 L43.270,37.708 C43.675,38.114 43.675,38.771 43.270,39.176 L39.551,42.900 C37.191,45.264 33.364,45.264 31.004,42.900 L24.906,36.795 L21.491,40.215 C21.086,40.620 20.430,40.620 20.025,40.215 L17.951,38.138 C17.719,37.905 17.612,37.576 17.660,37.251 L18.624,30.501 L12.590,24.460 C11.040,22.908 11.040,20.390 12.590,18.838 C14.141,17.285 16.654,17.285 18.205,18.838 L24.077,24.716 L35.851,18.820 C36.250,18.620 36.732,18.699 37.048,19.015 L39.122,21.092 C39.527,21.497 39.527,22.155 39.122,22.560 L30.521,31.173 L35.622,36.277 L40.262,35.347 ZM20.758,38.012 L23.440,35.327 L20.454,32.337 L19.784,37.036 L20.758,38.012 ZM34.541,38.138 L28.318,31.907 C27.914,31.501 27.914,30.844 28.318,30.438 L36.919,21.826 L36.107,21.013 L24.333,26.910 C23.934,27.109 23.452,27.031 23.136,26.714 L16.735,20.306 C16.379,19.949 15.897,19.749 15.394,19.749 C14.347,19.750 13.498,20.600 13.499,21.649 C13.496,22.154 13.695,22.638 14.051,22.995 L20.449,29.401 L25.635,34.593 L32.464,41.432 C34.014,42.984 36.528,42.984 38.078,41.432 L41.064,38.442 L40.115,37.492 L35.474,38.421 C35.135,38.488 34.786,38.382 34.541,38.138 Z"></path>
                </svg>
                <span className="brand brand-dark">Travila</span>
              </a>
            </div>
            <div className="bootstrap-select__custom col-xl-8 d-block d-lg-flex justify-content-xl-end align-items-center">
              <div className="mb-4 mb-lg-0">
                <ul className="d-flex list-unstyled mb-0">
                  {paymentIcons.map((icon, index) => (
                    <li key={index} className="mr-2 ml-0">
                      <img
                        className="max-width-40"
                        src={icon.src}
                        alt={icon.alt}
                      />
                    </li>
                  ))}
                </ul>
              </div>
              <select
                className="footer-dropdown-select min-width-256 w-100 w-md-auto border-radius-3 mb-3 mb-md-0 mr-md-5"
                data-style="border border-width-2 border-color-8"
              >
                {languageOptions.map((option, index) => (
                  <option
                    key={index}
                    className="border-bottom py-2 font-size-16"
                    value={option.value}
                    selected={option.value === 'one'}
                  >
                    {option.text}
                  </option>
                ))}
              </select>
              <select
                className="footer-dropdown-select pr-0 max-width-155 w-100 border-radius-3"
                data-style="border border-width-2 border-color-8"
              >
                {currencyOptions.map((option, index) => (
                  <option
                    key={index}
                    className="border-bottom py-2 font-size-16"
                    value={option.value}
                    selected={option.value === 'one'}
                  >
                    {option.text}
                  </option>
                ))}
              </select>
            </div>
          </div>
        </div>
      </div>
      <div className="space-1">
        <div className="container">
          <div className="d-lg-flex d-md-block justify-content-between align-items-center">
            <p className="mb-3 mb-lg-0 text-gray-1">
              &copy; 2024 Travila. All rights reserved
            </p>
            <ul className="list-inline mb-0">
              {bottomLinks.map((link, index) => (
                <li key={index} className="list-inline-item pb-3 pb-md-0">
                  <a
                    className="list-group-item-action text-decoration-on-hover pr-3"
                    href={link.link}
                  >
                    {link.text}
                  </a>
                </li>
              ))}
            </ul>
          </div>
        </div>
      </div>
    </footer>
  );
};

export default Footer;